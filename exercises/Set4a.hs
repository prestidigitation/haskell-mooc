...
longest xs = go xs []
  where
    go [] currentLongest = currentLongest
    go (x:xs) currentLongest
      | length x > length currentLongest = go xs x
      | length x == length currentLongest = if head x <= head currentLongest then go xs x else go xs currentLongest
      | otherwise = go xs currentLongest
...
--   where
--     go acc [] = acc
--     go acc (x:xs) = go (Map.insertWith (+) x 1 acc) xs

-- Challenge 1 solution
-- freqs xs = go Map.empty xs
--   where
--     go acc [] = acc
--     go acc (x:xs) = go (Map.alter f x acc) xs
...
transfer from to amount bank
  ...
  | amount < 0 = bank
  | fromAmount < amount = bank
  | otherwise = Map.insert to (toAmount + amount) (Map.insert from (fromAmount - amount) bank)
  where
    fromAmount = bank Map.! from
    toAmount = bank Map.! to
